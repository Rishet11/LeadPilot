...
def test_dodo_webhook_events_list_allows_admin(client):
    response = client.get("/api/webhooks/dodo/events")
    assert response.status_code == 200
    assert isinstance(response.json(), list)
...
def test_dodo_webhook_events_list_blocks_non_admin(client):
    previous_override = app.dependency_overrides.get(get_current_customer)
    app.dependency_overrides[get_current_customer] = lambda: {
        "id": 1,
        "name": "Test Customer",
...
def test_dodo_webhook_retry_blocks_non_admin(client):
    previous_override = app.dependency_overrides.get(get_current_customer)
    app.dependency_overrides[get_current_customer] = lambda: {
        "id": 1,
        "name": "Test Customer",
    ...
    try:
        response = client.post("/api/webhooks/dodo/retry/test-event")
        assert response.status_code == 403
        assert response.json()["detail"] == "Admin access required"
    finally:
        if previous_override is None:
